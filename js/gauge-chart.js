...
function gaugeChart(minValue, maxValue, speed, fmt) {
  var margin = {top: 0, right: 65, bottom: 10, left: 65},
      ...
      innerRadius = 60,
      outerRadius = 80,
      dataDomain = [minValue, (minValue + maxValue) / 2, maxValue],
      labelPad = 10,
      dataValue = function(d) { return +d; },
      colorScale = d3.scaleLinear(),
      arcScale = d3.scaleLinear(),
      colorOptions = ["#080808", "#4682b4", "#4682ff"],
      arc = d3.arc();
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      arcGEnter.selectAll(".lines").data(arcScale.ticks(1).map(function(d) {
        return { score: d };
      })).enter()
        .append("path")
          .attr("class", "lines");
      arcGEnter.selectAll(".ticks").data(arcScale.ticks(1))
        .enter().append("text")
          .attr("class", "ticks");
      ...
      svg.select("text.arc-label")
        ...
        .text(function(d) { return d3.format(fmt)(d.score); });
